"""bootstrap core schema tables

Revision ID: 20260413_0002
Revises: 20260413_0001
Create Date: 2026-04-13 11:20:00
"""

from collections.abc import Sequence

from alembic import op

# revision identifiers, used by Alembic.
revision: str = "20260413_0002"
down_revision: str | None = "20260413_0001"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None


def upgrade() -> None:
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS users (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            display_name VARCHAR(100) NOT NULL,
            phone_ciphertext TEXT,
            password_hash TEXT,
            status VARCHAR(20) NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'disabled')),
            last_login_at BIGINT,
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            updated_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS user_identities (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            user_id BIGINT NOT NULL REFERENCES users(id),
            provider VARCHAR(30) NOT NULL CHECK (provider IN ('wechat_mp')),
            openid_ciphertext TEXT NOT NULL,
            unionid_ciphertext TEXT,
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            updated_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            UNIQUE (user_id, provider)
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS families (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            name VARCHAR(100) NOT NULL,
            timezone VARCHAR(64) NOT NULL DEFAULT 'Asia/Shanghai',
            status VARCHAR(20) NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'archived')),
            created_by BIGINT REFERENCES users(id),
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            updated_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS family_members (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            family_id BIGINT NOT NULL REFERENCES families(id),
            user_id BIGINT NOT NULL REFERENCES users(id),
            role VARCHAR(20) NOT NULL CHECK (role IN ('owner', 'caregiver', 'viewer')),
            status VARCHAR(20) NOT NULL DEFAULT 'active' CHECK (status IN ('pending', 'active', 'removed')),
            invited_by BIGINT REFERENCES users(id),
            joined_at BIGINT,
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            updated_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            UNIQUE (family_id, user_id)
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS babies (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            family_id BIGINT NOT NULL REFERENCES families(id),
            name VARCHAR(100) NOT NULL,
            nickname VARCHAR(100),
            gender VARCHAR(20) NOT NULL DEFAULT 'unknown' CHECK (gender IN ('male', 'female', 'unknown')),
            birth_date BIGINT NOT NULL,
            due_date BIGINT,
            birth_weight_g INTEGER,
            birth_height_cm NUMERIC(6,2),
            birth_head_circumference_cm NUMERIC(6,2),
            status VARCHAR(20) NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'archived')),
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            updated_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS growth_events (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            family_id BIGINT NOT NULL REFERENCES families(id),
            baby_id BIGINT NOT NULL REFERENCES babies(id),
            event_type VARCHAR(30) NOT NULL CHECK (
                event_type IN ('feeding', 'excretion', 'measurement', 'sleep', 'medication', 'vaccine', 'milestone')
            ),
            occurred_at BIGINT NOT NULL,
            start_at BIGINT,
            end_at BIGINT,
            timezone VARCHAR(64) NOT NULL DEFAULT 'Asia/Shanghai',
            source VARCHAR(20) NOT NULL DEFAULT 'miniapp' CHECK (source IN ('miniapp', 'h5', 'admin', 'system')),
            notes TEXT,
            payload_snapshot JSONB NOT NULL DEFAULT '{}'::jsonb,
            status VARCHAR(20) NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'deleted')),
            created_by BIGINT REFERENCES users(id),
            updated_by BIGINT REFERENCES users(id),
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            updated_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            deleted_at BIGINT
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS feeding_events (
            event_id BIGINT PRIMARY KEY REFERENCES growth_events(id),
            mode VARCHAR(30) NOT NULL CHECK (mode IN ('breast_milk', 'bottle_breast_milk', 'formula', 'solid_food', 'water')),
            volume NUMERIC(10,2) NOT NULL CHECK (volume > 0),
            unit VARCHAR(10) NOT NULL DEFAULT 'ml',
            is_night BOOLEAN NOT NULL DEFAULT FALSE,
            extra JSONB NOT NULL DEFAULT '{}'::jsonb
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS excretion_events (
            event_id BIGINT PRIMARY KEY REFERENCES growth_events(id),
            excretion_type VARCHAR(20) NOT NULL CHECK (excretion_type IN ('urine', 'stool', 'mixed')),
            color VARCHAR(30),
            texture VARCHAR(30),
            is_abnormal BOOLEAN NOT NULL DEFAULT FALSE,
            extra JSONB NOT NULL DEFAULT '{}'::jsonb
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS measurement_events (
            event_id BIGINT PRIMARY KEY REFERENCES growth_events(id),
            weight_g INTEGER CHECK (weight_g > 0),
            height_cm NUMERIC(6,2),
            head_circumference_cm NUMERIC(6,2),
            hand_length_cm NUMERIC(6,2),
            leg_length_cm NUMERIC(6,2),
            temperature_c NUMERIC(4,2),
            extra JSONB NOT NULL DEFAULT '{}'::jsonb
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS sleep_events (
            event_id BIGINT PRIMARY KEY REFERENCES growth_events(id),
            duration_minutes INTEGER NOT NULL CHECK (duration_minutes >= 0),
            night_wake_count INTEGER NOT NULL DEFAULT 0 CHECK (night_wake_count >= 0),
            quality_tag VARCHAR(20),
            extra JSONB NOT NULL DEFAULT '{}'::jsonb
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS medication_events (
            event_id BIGINT PRIMARY KEY REFERENCES growth_events(id),
            medicine_name VARCHAR(120) NOT NULL,
            dosage NUMERIC(10,2),
            dosage_unit VARCHAR(20),
            planned_at BIGINT,
            taken_at BIGINT,
            execution_status VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (execution_status IN ('pending', 'taken', 'skipped')),
            extra JSONB NOT NULL DEFAULT '{}'::jsonb
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS vaccine_events (
            event_id BIGINT PRIMARY KEY REFERENCES growth_events(id),
            vaccine_name VARCHAR(120) NOT NULL,
            planned_at BIGINT,
            completed_at BIGINT,
            institution_name VARCHAR(120),
            batch_no VARCHAR(80),
            status VARCHAR(20) NOT NULL DEFAULT 'planned' CHECK (status IN ('planned', 'completed', 'missed')),
            extra JSONB NOT NULL DEFAULT '{}'::jsonb
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS milestone_events (
            event_id BIGINT PRIMARY KEY REFERENCES growth_events(id),
            milestone_type VARCHAR(50) NOT NULL,
            description TEXT,
            extra JSONB NOT NULL DEFAULT '{}'::jsonb
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS daily_summaries (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            family_id BIGINT NOT NULL REFERENCES families(id),
            baby_id BIGINT NOT NULL REFERENCES babies(id),
            summary_date BIGINT NOT NULL,
            feeding_total_ml INTEGER NOT NULL DEFAULT 0,
            feeding_breakdown JSONB NOT NULL DEFAULT '{}'::jsonb,
            excretion_count_total INTEGER NOT NULL DEFAULT 0,
            excretion_breakdown JSONB NOT NULL DEFAULT '{}'::jsonb,
            sleep_total_minutes INTEGER NOT NULL DEFAULT 0,
            last_measurement_snapshot JSONB NOT NULL DEFAULT '{}'::jsonb,
            alert_count INTEGER NOT NULL DEFAULT 0,
            ai_summary TEXT,
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            updated_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            UNIQUE (family_id, baby_id, summary_date)
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS weekly_summaries (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            family_id BIGINT NOT NULL REFERENCES families(id),
            baby_id BIGINT NOT NULL REFERENCES babies(id),
            week_start BIGINT NOT NULL,
            summary_payload JSONB NOT NULL DEFAULT '{}'::jsonb,
            ai_summary TEXT,
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            updated_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            UNIQUE (family_id, baby_id, week_start)
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS metric_snapshots (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            family_id BIGINT NOT NULL REFERENCES families(id),
            baby_id BIGINT NOT NULL REFERENCES babies(id),
            metric_code VARCHAR(50) NOT NULL,
            bucket_type VARCHAR(20) NOT NULL CHECK (bucket_type IN ('day', 'week', 'month')),
            bucket_date BIGINT NOT NULL,
            metric_value NUMERIC(12,2) NOT NULL,
            metric_unit VARCHAR(20),
            tags JSONB NOT NULL DEFAULT '{}'::jsonb,
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            updated_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            UNIQUE (baby_id, metric_code, bucket_type, bucket_date)
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS alert_rules (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            family_id BIGINT NOT NULL REFERENCES families(id),
            rule_type VARCHAR(50) NOT NULL,
            threshold_value NUMERIC(12,2),
            window_hours INTEGER,
            window_days INTEGER,
            severity VARCHAR(20) NOT NULL CHECK (severity IN ('info', 'warning', 'high')),
            enabled BOOLEAN NOT NULL DEFAULT TRUE,
            config JSONB NOT NULL DEFAULT '{}'::jsonb,
            created_by BIGINT REFERENCES users(id),
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            updated_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS alert_events (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            family_id BIGINT NOT NULL REFERENCES families(id),
            baby_id BIGINT NOT NULL REFERENCES babies(id),
            rule_id BIGINT REFERENCES alert_rules(id),
            severity VARCHAR(20) NOT NULL CHECK (severity IN ('info', 'warning', 'high')),
            title VARCHAR(200) NOT NULL,
            content TEXT NOT NULL,
            status VARCHAR(20) NOT NULL DEFAULT 'open' CHECK (status IN ('open', 'acknowledged', 'resolved')),
            triggered_at BIGINT NOT NULL,
            acknowledged_at BIGINT,
            resolved_at BIGINT,
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            updated_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS ai_conversations (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            family_id BIGINT NOT NULL REFERENCES families(id),
            baby_id BIGINT NOT NULL REFERENCES babies(id),
            asked_by BIGINT REFERENCES users(id),
            question TEXT NOT NULL,
            answer TEXT,
            model_name VARCHAR(100),
            context_window VARCHAR(50),
            status VARCHAR(20) NOT NULL CHECK (status IN ('succeeded', 'failed')),
            error_message TEXT,
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS ai_messages (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            conversation_id BIGINT NOT NULL REFERENCES ai_conversations(id),
            role VARCHAR(20) NOT NULL CHECK (role IN ('system', 'user', 'assistant')),
            content TEXT NOT NULL,
            token_count INTEGER,
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS export_tasks (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            family_id BIGINT NOT NULL REFERENCES families(id),
            baby_id BIGINT NOT NULL REFERENCES babies(id),
            report_type VARCHAR(20) NOT NULL CHECK (report_type IN ('daily', 'weekly', 'monthly', 'custom')),
            date_from BIGINT NOT NULL,
            date_to BIGINT NOT NULL,
            status VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'running', 'succeeded', 'failed')),
            object_key VARCHAR(255),
            download_url TEXT,
            expires_at BIGINT,
            error_message TEXT,
            requested_by BIGINT REFERENCES users(id),
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            updated_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS task_jobs (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            job_type VARCHAR(50) NOT NULL,
            payload JSONB NOT NULL DEFAULT '{}'::jsonb,
            status VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'running', 'succeeded', 'failed')),
            retry_count INTEGER NOT NULL DEFAULT 0,
            max_retries INTEGER NOT NULL DEFAULT 3,
            run_after BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            locked_at BIGINT,
            locked_by VARCHAR(100),
            error_message TEXT,
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT,
            updated_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT
        );
        """
    )
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS operation_logs (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            family_id BIGINT REFERENCES families(id),
            operator_user_id BIGINT REFERENCES users(id),
            resource_type VARCHAR(50) NOT NULL,
            resource_id BIGINT,
            action VARCHAR(30) NOT NULL,
            source VARCHAR(20) NOT NULL CHECK (source IN ('miniapp', 'h5', 'admin', 'system')),
            old_value JSONB,
            new_value JSONB,
            created_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT
        );
        """
    )

    op.execute("CREATE INDEX IF NOT EXISTS idx_growth_events_baby_occurred_at ON growth_events (baby_id, occurred_at DESC);")
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_growth_events_family_type_occurred_at ON growth_events (family_id, event_type, occurred_at DESC);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_daily_summaries_baby_date ON daily_summaries (baby_id, summary_date DESC);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_metric_snapshots_baby_metric_bucket ON metric_snapshots (baby_id, metric_code, bucket_date);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_alert_events_baby_status_triggered ON alert_events (baby_id, status, triggered_at DESC);"
    )
    op.execute("CREATE INDEX IF NOT EXISTS idx_task_jobs_status_run_after ON task_jobs (status, run_after);")


def downgrade() -> None:
    # 保守策略：历史环境不自动 drop 业务表。
    pass
