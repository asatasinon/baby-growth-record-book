...
| v0.3.0 | 2026-04-12 | Codex | 增加 API 层 ID 字符串与数据库 `bigint` 转换约定。 |
| v0.4.0 | 2026-04-12 | Codex | 增加转换层治理要求与非法 ID 错误约定。 |
...
- API 边界不直接暴露数据库数值 ID，统一返回字符串 ID，由应用层执行字符串与 `BIGINT` 的双向转换。
- 枚举值默认通过 `CHECK` 或 PostgreSQL `ENUM` 管理。
- 非法 ID 输入统一返回 `INVALID_ARGUMENT`，并在 `data.field` 中明确字段名。
