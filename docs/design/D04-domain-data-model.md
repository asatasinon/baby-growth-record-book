...
| v0.3.0 | 2026-04-12 | Codex | 增加 API 边界 ID 转换规则（数据库 `bigint`，接口字符串）。 |
| v0.4.0 | 2026-04-12 | Codex | 增加“统一转换层”要求，禁止业务层分散转换。 |
...
- API 边界统一返回字符串 ID，入参字符串 ID 在应用层转换为 `bigint` 后参与查询和写入。
- ID 转换必须收敛在统一 schema/adapter 层，禁止在领域服务中散落转换逻辑。
